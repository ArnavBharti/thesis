...
class ExperimentConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if min(
            self.pilot_per_tier,
            self.main_per_tier,
            self.mechanism_per_tier,
            self.ablation_per_tier,
        ) < 1:
            raise ValueError("subset sizes must be positive")
        if self.mechanism_per_tier > self.main_per_tier:
            raise ValueError("mechanism sample cannot be larger than the main sample")
        if self.ablation_per_tier > self.mechanism_per_tier:
            raise ValueError("ablation sample cannot be larger than the mechanism sample")
