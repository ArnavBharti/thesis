...
def main() -> int:
    ...
    parser.add_argument("--cache-dir", type=Path, help="Hugging Face cache in Sharanga scratch")
    ...
    cache_directory = arguments.cache_dir.resolve() if arguments.cache_dir else None
    for model in models:
        plan = {
            "model": model.name,
            "repo_id": model.resolved_model_id,
            "revision": model.revision,
            "cache_dir": str(cache_directory) if cache_directory else os.environ.get("HF_HOME", "default"),
        }
        if arguments.dry_run:
            print(json.dumps({**plan, "status": "planned"}, sort_keys=True))
            continue
        snapshot = ensure_snapshot(model, cache_directory, verify_only=arguments.verify_only)
