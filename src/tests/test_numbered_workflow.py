...
class NumberedWorkflowTests(unittest.TestCase):
    ...
    def test_slurm_job_runs_the_same_numbered_script_once(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory, patch("lib.slurm.ROOT", Path(directory)):
            ...
            self.assertIn("g++ -print-file-name=libstdc++.so.6", text)
            ...
            self.assertIn('ln -sfn "$CUDA_HOME/lib/libcudart.so.13"', text)
            self.assertIn('ln -sfn "$CUDA_HOME/lib/libnvrtc.so.13"', text)
            self.assertIn('export LIBRARY_PATH="$CUDA_LINK_DIR', text)
            self.assertIn('export LD_LIBRARY_PATH="$CUDA_HOME/lib:$LD_LIBRARY_PATH"', text)
