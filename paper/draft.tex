...
\begin{abstract}
This study evaluates whether a large language model preserves Sudoku-solving ability when the nine values are renamed by bijective symbol systems. Sudoku provides exact, deterministic scoring: a returned grid must preserve every clue and satisfy every row, column, and $3\times3$ box. A certified dataset contains 300 unique-solution puzzles divided equally among easy, medium, and hard tiers. The selected model, GPT-OSS-120B, uses hidden Harmony analysis while only its final channel is scored. It solved all five qualification puzzles. On the frozen 540-request main benchmark, it solved \MainCorrect{} requests (\MainAccuracy{}) with no operational failures. Accuracy ranged from 61.7\% for Greek letters to 75.0\% for Arabic digits and uppercase Latin, while difficulty accuracy declined from 95.0\% on easy puzzles to 67.2\% on medium and 44.4\% on hard puzzles. In a 135-condition input/output-cross experiment, all 75 basic symbol controls were correct, whereas Sudoku accuracy was 12/15 for both Arabic-to-Arabic and Greek-to-Greek, 10/15 for Greek-to-Arabic, and 11/15 for Arabic-to-Greek. A controlled token-length experiment was non-monotonic: one-, two-, and three-token labels scored 9/15, 13/15, and 10/15. These results demonstrate representation sensitivity but do not reduce it to Unicode handling, transformed-output generation, or token length alone.
\end{abstract}
...
\section{Conclusion}
This work implements an exactly scored test of representation invariance in LLM Sudoku solving. GPT-OSS-120B passed all qualification requests, solved 47/60 pilot requests, and solved 372/540 requests in the complete frozen main benchmark without operational failures. Main accuracy differed across bijective representations of the same puzzle set, ranging from 61.7\% to 75.0\%, and declined from 95.0\% on easy puzzles to 44.4\% on hard puzzles. Among the 45 puzzles solved in Arabic, transformed-representation retention ranged from 68.9\% to 82.2\%. The completed input/output-cross experiment found perfect basic symbol-control performance but lower Sudoku accuracy, with no simple penalty attributable solely to Greek output. The token-length experiment was non-monotonic, with the two-token alphabet outperforming both the one- and three-token alphabets. These findings demonstrate that successful Sudoku solving by GPT-OSS-120B is not fully invariant to value renaming and that the observed effect reflects more than basic Unicode manipulation or label token count.
